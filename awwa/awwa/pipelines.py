...
class AWWAPipeline(object):

    def open_spider(self, spider):
        self.connection = sqlite3.connect('../bookawwa.db')
        self.cursor = self.connection.cursor()
        self.cursor.execute("""CREATE TABLE IF NOT EXISTS AWWA \
    ...
    def process_item(self, item, spider):
        self.cursor.execute(
            "select isbn from AWWA where isbn=%s" % item['isbn'])
        ...
        if result:
            ...
            self.cursor.execute(
                """insert into AWWA (url, title, autor, date, isbn,
                ...
                (item['url'], item['title'], item["autor"],
                 ...
                 item["pages"], item["serie"], cat, item["image"],
